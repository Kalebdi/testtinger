...
function blockShuffle(data, nBlocks, seed) {
  ...
  for (let i=0;i<nBlocks;i++) {
    const sl = data.slice(i*bSz, (i+1)*bSz);
    if (sl.length) blocks.push(sl);
  }
...
function makeOpcodeTable() {
  ...
  while (ids.length < names.length) {
    const x = ri(1, 120);
    if (!used.has(x)) { ids.push(x); used.add(x); }
  }
  const T = {};
  names.forEach((n,i) => { T[n] = ids[i]; });
  const fakes = [];
  while (fakes.length < 12) {
    const x = ri(130, 210);
    if (!used.has(x)) { fakes.push(x); used.add(x); }
  }
  T._fakes = fakes;
  return T;
}
...
function junkLine() {
  ...
  switch(t) {
    ...
    case 2: return `local ${a}=type(nil) local ${b}=#${a}`;
...
do
  local ${vAT}=rawget(${vEnv},"hookfunction") or rawget(${vEnv},"hookfunc") or rawget(${vEnv},"replaceclosure")
  if ${vAT}~=nil then _kick() return end
end
do
  local ${vExec}=rawget(${vEnv},${xRf}) or rawget(${vEnv},${xWf}) or rawget(${vEnv},${xSyn}) or rawget(${vEnv},${xFlux})
...
${bigJunk(2)}
local ${vX1}=${toLuaEscape(xorKey.slice(0,xM))}
local ${vX2}=${toLuaEscape(xorKey.slice(xM))}
...
${bigJunk(2)}
local ${vK1}=${toLuaEscape(rc4Key.slice(0,kM1))}
local ${vK2}=${toLuaEscape(rc4Key.slice(kM1,kM2))}
local ${vK3}=${toLuaEscape(rc4Key.slice(kM2))}
...
local ${vIns}={} for ${vIdx}=1,${vI32}() do
  ...
  if _at==1 then
    local _lo=${vU8}() local _hi=${vU8}()
    _av=_lo+_hi*256 if _av>=32768 then _av=_av-65536 end
  elseif _at==2 then _av=${vI32}()
  elseif _at==3 then
    local _fb={} for _k=1,8 do _fb[_k]=${vU8}() end
    local _ok,_fv=pcall(string.unpack,">d",string.char(table.unpack(_fb)))
    _av=_ok and _fv or 0
  end
...
while ${vRun} do
  ...
  if ${vOp}==${A(OPC.LOAD_CONST)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=${vCons}[${vA}+1]
  elseif ${vOp}==${A(OPC.LOAD_NUMBER)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=${vA}
  elseif ${vOp}==${A(OPC.LOAD_NIL)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=nil
  elseif ${vOp}==${A(OPC.LOAD_TRUE)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=true
  elseif ${vOp}==${A(OPC.LOAD_FALSE)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=false
  elseif ${vOp}==${A(OPC.LOAD_VAR)} then
    ${vTop}=${vTop}+1 ${vStk}[${vTop}]=${vVars}[${vA}]
  elseif ${vOp}==${A(OPC.STORE_VAR)} then
  ...
  elseif ${vOp}==${A(OPC.BINARY_ADD)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]+_b
  elseif ${vOp}==${A(OPC.BINARY_SUB)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]-_b
  elseif ${vOp}==${A(OPC.BINARY_MUL)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]*_b
  elseif ${vOp}==${A(OPC.BINARY_DIV)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]/_b
  elseif ${vOp}==${A(OPC.BINARY_MOD)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]%_b
  elseif ${vOp}==${A(OPC.BINARY_POW)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]^_b
  elseif ${vOp}==${A(OPC.BINARY_CONCAT)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=tostring(${vStk}[${vTop}])..tostring(_b)
  elseif ${vOp}==${A(OPC.BINARY_EQ)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]==_b
  elseif ${vOp}==${A(OPC.BINARY_NE)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]~=_b
  elseif ${vOp}==${A(OPC.BINARY_LT)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]<_b
  elseif ${vOp}==${A(OPC.BINARY_LE)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}]<=_b
  elseif ${vOp}==${A(OPC.BINARY_AND)} then
    local _b=${vStk}[${vTop}] ${vStk}[${vTop}]=nil ${vTop}=${vTop}-1
    ${vStk}[${vTop}]=${vStk}[${vTop}] and _b
