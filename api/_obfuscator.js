...
function randomBytes(n) {
  ...
  catch { const b = Buffer.alloc(n); crypto.getRandomValues(b); return b; }
}
function ri(a, b) { return Math.floor(Math.random() * (b - a + 1)) + a; }

// Lua-safe variable names (letters + underscore only)
function v() {
...
// Arithmetic obfuscation: replaces a number with a complex expression
function Arith(n) {
  const t = ri(0, 7);
  const a = ri(1, 99999);
  const b = ri(1, 99999);
  switch(t) {
    ...
    case 6: return `${-a + (n + a)}`;
    case 7: return `bit32.band(${n | a},${a})`;
    default: return `${n}`;
  }
}
...
function buildStringPool(code) {
  ...
  const G = (si) => {
    ...
    return `${vTbl}[${Arith(si+1)}+${a}-${a}]`;  // subtle arithmetic on index
  };
...
// ---------- VM / EXECUTOR ----------
function buildVM(vmOps, fakeOps, stXorKey, G, tableChecksum, payloadChecksum) {
  ...
  const vI = v(), vJ = v(), vK = v(), vSI = v(), vSJ = v(), vTmp = v(), vOut = v2();
  ...
  const cases = [
    ...
    { op: OP_TBLCHK, body: [
      `local ${vTblConcat}=""`,
      `for ${vI}=1,#${G(7)} do ${vTblConcat}=${vTblConcat}..${G(7)}[${vI}] end`,
  ...
  const allOps = [...vmOps.map(o => ({ ...o, fake: false })), ...fakeOps.map(f => ({ ...f, fake: true }))]
...
function obfuscateV2(code, options = {}) {
  ...
  const { tbl, G, pool } = buildStringPool(code);
  ...
  const vm = buildVM(vmOps, fakeOps, stXorKey, G, tableChecksum, payloadChecksum);
