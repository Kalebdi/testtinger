...
function makeOpcodeTable() {
  ...
  const fakes=[];
  while (fakes.length<10) { const x=ri(130,200); if(!used.has(x)){fakes.push(x);used.add(x);} }
  T._fakes=fakes;
  return T;
}

function junkLine() {
  const a=v(), b=v(), t=ri(0,4);
  switch(t) {
...
function bigJunk(n) {
  const p=[]; for(let i=0;i<n;i++) p.push(junkLine()); return p.join(' ');
}
...
function lex(src) {
  ...
  while (i<src.length) {
    ...
    if (/[0-9]/.test(src[i])||(src[i]==='.'&&/[0-9]/.test(src[i+1]||''))) {
      let s='';
      if (src.slice(i,i+2).toLowerCase()==='0x'){s='0x';i+=2;while(/[0-9a-fA-F]/.test(src[i]||''))s+=src[i++];}
      else{while(/[0-9._e+\-]/.test(src[i]||''))s+=src[i++];}
...
function emitVM(shuffleResult, rc4Key, xorKey, rawChecksum, OPC) {
  ...
  const xInst=makeXorStr('Instance'), xDM=makeXorStr('DataModel');
  ...
  const fakeBranches=OPC._fakes.slice(0,5).map(fop=>{
    const d=v(); return `elseif ${vOp}==${A(fop)} then local ${d}=${A(0)}`;
  }).join(' ');
...
for ${vIdx}=0,255 do ${vRJ}=(${vRJ}+${vS}[${vIdx}]+string.byte(${vRKey},(${vIdx}%_rkl)+1))%256 ${vS}[${vIdx}],${vS}[${vRJ}]=${vS}[${vRJ}],${vS}[${vIdx}] end
${vRKey}=nil
